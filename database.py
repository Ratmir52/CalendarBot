...
class DataBase():
    ...
    def upd_chat(self, new_id, usr_id):
        cur.execute(f"SELECT msg, fnsh_t FROM {self.r_table} WHERE id = ?", (usr_id,))
        rows = cur.fetchall()

        for msg, fnsh_t in rows:

            exists = cur.execute(
                f"SELECT 1 FROM {self.r_table} WHERE id = ? AND chat_id = ? AND msg = ? AND fnsh_t = ?",
                (usr_id, new_id, msg, fnsh_t)
            ).fetchone()

            if exists:

                cur.execute(
                    f"DELETE FROM {self.r_table} WHERE id = ? AND chat_id != ? AND msg = ? AND fnsh_t = ?",
                    (usr_id, new_id, msg, fnsh_t)
                )
            else:
                # записи нет → просто обновляем
                cur.execute(
                    f"UPDATE {self.r_table} SET chat_id = ? WHERE id = ? AND msg = ? AND fnsh_t = ?",
                    (new_id, usr_id, msg, fnsh_t)
                )
